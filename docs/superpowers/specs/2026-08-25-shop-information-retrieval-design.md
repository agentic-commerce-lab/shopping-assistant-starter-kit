...
| R3 | **Below a similarity threshold there is no passage at all.** The model is told nothing was found and must say so | Vector search always returns something — the nearest passage, whether or not it answers the question. "Can I pay with Bitcoin?" against a privacy policy returns a paragraph about payment data processing, and a paraphrase of that is a confident wrong answer. The threshold is what turns "nearest" into "relevant", and it is the same discipline `no_match_not_absence` already enforces for products |
| R4 | **The threshold is a constant, not a merchant setting** | No merchant calibrates a cosine distance. The value is ours to get right, which means ours to measure — see R5 |
...
| R9 | **Documents are first-class entities**, with a name, type, upload date, status, chunk count — and the extracted text | The identity R8 needs, the list the admin module shows, and the reason re-indexing needs no file access. Storing the text rather than the bytes also removes the question of where uploaded files live |
...
- **The threshold ships uncalibrated.** R5's trace is the plan for fixing that, not a substitute for
  having done it.
