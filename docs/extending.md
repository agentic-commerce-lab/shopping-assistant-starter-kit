...
| `OrderHistoryReader` | `orders(OrderQuery): list<OrderSummary>`, `order(string $orderNumber): ?OrderDetail` | `list_orders` is never constructed, so a signed-in shopper asking for their own orders or invoices is declined or escalated — which is the shipped behaviour for every gateway that does not implement it. **Implement it only by reading through Shopware's order route**: under Commercial's B2B Components every employee of one company presents the same customer id, and the filter that separates them is a decoration of that service. Type the dependency `AbstractOrderRoute` but wire the concrete id `Shopware\Core\Checkout\Order\SalesChannel\OrderRoute` — decoration gives the decorator the decorated service's id, `AbstractOrderRoute` is not a registered service at all, and `…DecoratedOrderRoute.inner` is the undecorated original. A query against `order.repository`, or that `.inner` service, returns the whole company's orders to every employee, silently. `order()` has the same duty and one more: make the order number a **criteria filter on the route**, so the employee filter applies to the same query — and return `null` without saying whether the order was missing or forbidden, because telling those apart confirms that a colleague's order exists |

Implement all six unless you have a reason not to. `DalCommerceGateway` implements every one and is
