...
| `OrderHistoryReader` | `orders(int $limit): list<OrderSummary>` | `list_orders` is never constructed, so a signed-in shopper asking for their own orders or invoices is declined or escalated — which is the shipped behaviour for every gateway that does not implement it. **Implement it only by reading through Shopware's `AbstractOrderRoute`**: under Commercial's B2B Components every employee of one company presents the same customer id, and the filter that separates them is a decoration of that service. A query against `order.repository` returns the whole company's orders to every employee, silently |
