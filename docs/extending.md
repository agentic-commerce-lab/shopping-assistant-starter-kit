...
| `OrderHistoryReader` | `orders(int $limit): list<OrderSummary>` | `list_orders` is never constructed, so a signed-in shopper asking for their own orders or invoices is declined or escalated — which is the shipped behaviour for every gateway that does not implement it. **Implement it only by reading through Shopware's order route**: under Commercial's B2B Components every employee of one company presents the same customer id, and the filter that separates them is a decoration of that service. Type the dependency `AbstractOrderRoute` but wire the concrete id `Shopware\Core\Checkout\Order\SalesChannel\OrderRoute` — decoration gives the decorator the decorated service's id, `AbstractOrderRoute` is not a registered service at all, and `…DecoratedOrderRoute.inner` is the undecorated original. A query against `order.repository`, or that `.inner` service, returns the whole company's orders to every employee, silently |
