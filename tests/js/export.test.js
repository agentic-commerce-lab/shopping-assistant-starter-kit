...
test('the request goes to the export action', () => {
    assert.equal(
        exportRequest(api, ['a'.repeat(32)], 'csv').url,
...
test('the request carries the admin token, because the route is ACL-protected', () => {
    ...
    const { options } = exportRequest(api, [], 'csv');
...
test('the body carries exactly the ids and the format the server expects', () => {
    const { options } = exportRequest(api, ['a'.repeat(32), 'b'.repeat(32)], 'json');
...
test('the saved file is named for its format', () => {
    assert.equal(exportFileName('csv'), 'assistant-traces.csv');
    assert.equal(exportFileName('json'), 'assistant-traces.json');
});
