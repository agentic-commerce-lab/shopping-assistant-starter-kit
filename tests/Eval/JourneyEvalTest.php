...
final class JourneyEvalTest extends TestCase
{
    ...
    public function testJourneyMeetsItsAssertions(string $path): void
    {
        ...
        // so an unanswerable question competes against several plausible neighbours the way it would
        // in a real shop. Passed unconditionally: a journey that does not configure an embeddingModel
        // never touches it, and one that does must not be able to run without it — see
        // JourneyAttempt::shopInfoFactories().
        $runner = new JourneyRunner($settings, LargeCatalogFile::chosen(), null, __DIR__ . '/../Fixtures/shop_info_en');
