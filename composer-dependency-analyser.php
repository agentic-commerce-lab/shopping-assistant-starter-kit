...
$config = (new Configuration())
    ...
    // Plan 1 (this task) only scaffolds the project and pins the Symfony AI stack; no src/
    // code consumes these packages yet. Tasks 2-13 wire the commerce gateway, LLM platform,
    // tools, and HTTP client usage that will make these imports real. Remove these ignores
    // once that code lands and the analyser finds genuine usage.
    ->ignoreErrorsOnPackage('symfony/ai-agent', [ErrorType::UNUSED_DEPENDENCY])
    ->ignoreErrorsOnPackage('symfony/ai-generic-platform', [ErrorType::UNUSED_DEPENDENCY])
    ->ignoreErrorsOnPackage('symfony/http-client', [ErrorType::UNUSED_DEPENDENCY])
    ->ignoreErrorsOnPackage('symfony/http-client-contracts', [ErrorType::UNUSED_DEPENDENCY]);
